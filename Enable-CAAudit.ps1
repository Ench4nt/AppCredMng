<#
.SYNOPSIS
    Enables the Conditional Access Policy Audit feature on an existing
    Azure Automation Account.

.DESCRIPTION
    Imports the required Graph modules, creates Automation Variables,
    publishes the CA audit runbook, and creates a weekly schedule.
    Run this after Deploy-AutomationAccount.ps1 has created the
    Automation Account.
...
.PARAMETER SignInLookbackDays
    Days of sign-in logs to check for policy activity. Default: 30.

.PARAMETER ReportOnlyThresholdDays
    Flag policies in report-only mode longer than this. Default: 60.

.PARAMETER DisabledThresholdDays
    Flag policies disabled longer than this. Default: 30.

.PARAMETER LargeGroupThreshold
    Flag exclusion groups with more members than this. Default: 50.

.PARAMETER ScheduleTime
    Weekly run time in HH:mm (UTC). Default: 08:00.

.PARAMETER ScheduleDay
    Day of week for the schedule. Default: Monday.
...
Write-Host "Enabling CA Policy Audit on '$AutomationAccountName' ..." -ForegroundColor Cyan
...
# ─────────────────────────────────────────────
$requiredModules = @(
    @{ Name = 'Microsoft.Graph.Identity.SignIns'; Uri = 'https://www.powershellgallery.com/api/v2/package/Microsoft.Graph.Identity.SignIns/2.25.0' }
    @{ Name = 'Microsoft.Graph.Groups';           Uri = 'https://www.powershellgallery.com/api/v2/package/Microsoft.Graph.Groups/2.25.0' }
    @{ Name = 'Microsoft.Graph.Users';            Uri = 'https://www.powershellgallery.com/api/v2/package/Microsoft.Graph.Users/2.25.0' }
    @{ Name = 'Microsoft.Graph.Applications';     Uri = 'https://www.powershellgallery.com/api/v2/package/Microsoft.Graph.Applications/2.25.0' }
    @{ Name = 'Microsoft.Graph.Reports';          Uri = 'https://www.powershellgallery.com/api/v2/package/Microsoft.Graph.Reports/2.25.0' }
...
$variables = @(
    ...
    @{ Name = 'SignInLookbackDays';      Value = $SignInLookbackDays;      Encrypted = $false }
    @{ Name = 'ReportOnlyThresholdDays'; Value = $ReportOnlyThresholdDays; Encrypted = $false }
    @{ Name = 'DisabledThresholdDays';   Value = $DisabledThresholdDays;   Encrypted = $false }
    @{ Name = 'LargeGroupThreshold';     Value = $LargeGroupThreshold;     Encrypted = $false }
)
...
# ─────────────────────────────────────────────
$runbookName = 'Audit-CAPolicies'
$runbookPath = Join-Path $PSScriptRoot 'Runbook-AuditCAPolicies.ps1'
...
# ─────────────────────────────────────────────
$scheduleName = 'Weekly-CAAudit'
$startTime = [DateTime]::Parse($ScheduleTime)

$targetDay = [System.Enum]::Parse([System.DayOfWeek], $ScheduleDay)
$daysUntilTarget = (([int]$targetDay - [int][DateTime]::UtcNow.DayOfWeek + 7) % 7)
if ($daysUntilTarget -eq 0) { $daysUntilTarget = 7 }
$startDate = [DateTime]::UtcNow.Date.AddDays($daysUntilTarget).Add($startTime.TimeOfDay)
...
if (-not $existing) {
    Write-Host "  Creating weekly schedule ($ScheduleDay at $ScheduleTime UTC) ..." -ForegroundColor Cyan
    New-AzAutomationSchedule -ResourceGroupName $ResourceGroupName `
                             -AutomationAccountName $AutomationAccountName `
                             -Name $scheduleName -StartTime $startDate `
                             -WeekInterval 1 -DaysOfWeek $ScheduleDay `
...
Write-Host " CA Policy Audit Enabled!" -ForegroundColor Green
Write-Host "=================================================================" -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "  `$permissions = @('Policy.Read.All', 'AuditLog.Read.All', 'Group.Read.All', 'User.Read.All', 'Application.Read.All', 'Mail.Send')" -ForegroundColor White
...
Write-Host ""
Write-Host "Note: Requires Entra ID P1/P2 license for sign-in log access." -ForegroundColor Yellow
Write-Host ""
