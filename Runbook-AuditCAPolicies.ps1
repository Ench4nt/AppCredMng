<#
.SYNOPSIS
    Azure Automation runbook that audits Entra ID Conditional Access policies
    for security gaps, hygiene issues, and organizational best practices.

.DESCRIPTION
    Connects to Microsoft Graph using Managed Identity and performs a
    comprehensive audit of all Conditional Access policies in the tenant
    with 27 checks across security gaps, governance, and policy hygiene.
    Results are compiled into an HTML report and emailed via Graph.

    This is the Azure Automation runbook version of
    Audit-ConditionalAccessPolicies.ps1. It reads configuration from
    Automation Variables instead of script parameters and authenticates
    via Managed Identity.

.NOTES
    REQUIRED AUTOMATION VARIABLES
        MailFrom               - Sender mailbox UPN (e.g. alerts@contoso.com)
        MailTo                 - Comma-separated recipient addresses
        SignInLookbackDays     - Days of sign-in logs to check (default: 30)
        ReportOnlyThresholdDays - Flag report-only policies older than this (default: 60)
        DisabledThresholdDays  - Flag disabled policies older than this (default: 30)
        LargeGroupThreshold    - Flag exclusion groups larger than this (default: 50)

    REQUIRED GRAPH PERMISSIONS (on the Managed Identity)
        Policy.Read.All
        AuditLog.Read.All
        Group.Read.All
        User.Read.All
    ...
    REQUIRED MODULES (imported into the Automation Account)
...
# ─────────────────────────────────────────────
# 1. Read Automation Variables
# ─────────────────────────────────────────────
$MailFrom                = [string](Get-AutomationVariable -Name 'MailFrom')
$MailTo                  = [string](Get-AutomationVariable -Name 'MailTo') -split '\s*,\s*'
$SignInLookbackDays      = try { [int](Get-AutomationVariable -Name 'SignInLookbackDays') } catch { 30 }
$ReportOnlyThresholdDays = try { [int](Get-AutomationVariable -Name 'ReportOnlyThresholdDays') } catch { 60 }
$DisabledThresholdDays   = try { [int](Get-AutomationVariable -Name 'DisabledThresholdDays') } catch { 30 }
$LargeGroupThreshold     = try { [int](Get-AutomationVariable -Name 'LargeGroupThreshold') } catch { 50 }

Write-Output "Config: MailFrom=$MailFrom | MailTo=$($MailTo -join ', ')"
Write-Output "Config: SignInLookbackDays=$SignInLookbackDays | ReportOnlyThresholdDays=$ReportOnlyThresholdDays"
Write-Output "Config: DisabledThresholdDays=$DisabledThresholdDays | LargeGroupThreshold=$LargeGroupThreshold"
...
# ─────────────────────────────────────────────
# 3. Connect to Microsoft Graph (Managed Identity)
# ─────────────────────────────────────────────
...
Write-Output "Found $($policies.Count) CA policy(ies)."

Write-Output "Fetching named locations ..."
$namedLocations = Get-MgIdentityConditionalAccessNamedLocation -All -ErrorAction Stop
Write-Output "Found $($namedLocations.Count) named location(s)."
...
Write-Output "Running security gap checks ..."
...
Write-Output "Checking break-glass account governance ..."
$breakGlassAccounts = @()
$allUsers = $null
try {
...
$registrationPolicy = $enabledPolicies | Where-Object {
    $ua = $null
    try { $ua = $_.Conditions.UserActions } catch { }
...
# ── Per-policy checks ──
Write-Output "Running per-policy checks ..."
...
Write-Output "Checking for duplicate policies ..."
$policySignatures = @{}
...
Write-Output "Checking sign-in logs for policy activity (last $SignInLookbackDays days) ..."
...
Write-Output "Audit complete: $($findings.Count) finding(s)."
...
$tenantContext = (Get-MgContext).TenantId
...
"<p>Tenant: <strong>$tenantContext</strong> | Generated: <strong>$($now.ToString('yyyy-MM-dd HH:mm:ss')) UTC</strong></p>" +
...
$inventoryRows + "</table>" +
"<p class=`"note`">Generated by Runbook-AuditCAPolicies.ps1. " +
...
Write-Output "Audit report emailed from $MailFrom to $($MailTo -join ', ')."
...
Write-Output "Policies: $totalPolicies ($enforcedCount enforced, $reportOnlyCount report-only, $disabledCount disabled)"
Write-Output "Findings: $criticalCount critical, $highCount high, $mediumCount medium, $warningCount warning, $infoCount info"

if ($findings.Count -gt 0) {
    Write-Output ""
    Write-Output "Top findings:"
